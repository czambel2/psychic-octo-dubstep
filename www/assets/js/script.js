$(function () {
	...
	$('table.contains-data-cyclists').dataTable( {
		"oLanguage": { "sUrl": "/assets/js/vendor/jquery.dataTables.fr.json" },
		"iDisplayLength": 20,
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "/api/liste-cyclistes",
		"fnInitComplete": function(oSettings, json) {
			$('.dataTables_filter input').attr('placeholder', 'Rechercher…');
		...
		"fnDrawCallback": function(oSettings) {
			$(this).removeHighlight();
			var enteredValues = $('.dataTables_filter input').val().split(' ');
			for(i = 0; i < enteredValues.length; i++) {
				enteredValue = supprimerAccents(enteredValues[i]);
				if(enteredValue != '') {
					$(this).highlight(enteredValue);
				}
			}
		}
	});

	var supprimerAccents = function(str) {
